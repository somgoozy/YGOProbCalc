...
public class CardProperties {
	...
	public CardProperties(String name) {
		...
		File jsonInputFile = new File("cardinfo.json");
		...
		try {
			...
			while(itr.hasNext()) {
	            JsonNode card_data = itr.next();
	            //if found
	            if(card_data.path("name").asText().toLowerCase().equals(name.toLowerCase())) {
	...
	private void fill (JsonNode card_node) {
		...
		if (this.properties.contains(PROPERTIES.MONSTER)) {
			...
			//if link
				...
				//get link rating
				this.link_rating = card_node.path("linkval").asInt();
			...
			if(!this.properties.contains(PROPERTIES.PENDULUM)) {
				this.pend_scale = card_node.path("scale").asInt();
			}
		}
	}
}